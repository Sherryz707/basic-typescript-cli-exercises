...
async function CalMsg(msg: string) {
  const CalTitle = chalkAnimation.rainbow(msg);
  await sleep();
  CalTitle.stop();
}
interface CalAnswerObj {
...
function Operation(answer: CalAnswerObj): number | never {
...
async function CalMenu() {
  ...
  do {
    try {
      if (first) {
        await CalMsg(
          "Hello there! Its your friendly neighbourhood calculator Cal!"
        );
        first = false;
      } else {
  ...
  await CalMsg(
    "Its sad to see you go....but i'm sure we'll meet again because how long can you go without Maths :)"
  );
}
CalMenu();
