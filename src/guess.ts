...
async function Welcome() {
  ...
  console.log(
    `The computer will generate a random number between 1 and 10. You have to correctly guess which number was generates. You have ${chalk.bgYellow(
      "5 lives"
    )} after which the game will end and you will ${chalk.bgRed("lose...:(")}`
  );
}
interface guessAns {
  guess: number;
...
function processAnswer({ guess }: guessAns) {
  let win = false;
...
async function game() {
  let win = true;
  let lives = 5;
  let againPl: playAgain = { again: "Yes" };
